...
> **注意:** この仕組みは完成品のライブラリではなく、あくまで実装例です。  
...
- `StartInternal()` に開始時の手続きを実装する
- 手続きが完了したら `Complete()` を呼び出す
- `SkipInternal()` は開始後に呼び出される場合がある。呼び出された場合は直ちに終了状態に遷移しなければならない
- `SkipInternal()` の内部から `Complete()` は呼び出さない（呼び出しても無視される）
- `isFastForward` は、開始直後に `Skip()` が実行されることが確定している場合に `true` になる。Chainは開始時にこの値を参照することで、不要な手続き（アニメーション開始やリソース確保など）を省略できる
...
各Chainは `SkipInternal()` でスキップ時の終了処理を実装しているため、スキップが発生しても正しい最終状態に遷移します。スキップ処理を個別に書く必要はありません。
